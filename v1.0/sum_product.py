...
def sum_product_update_var(state, messages, sender_id, recipient_id):
    ...
    observations = {'1': y1_observation,
                    '2': y2_observation}
    outgoing_message = SumProductVariableNode(sender_id,
            messages, observations[variable_index]).update_edge_message(recipient_id)
    return outgoing_message

def sum_product_update_fac(state, messages, sender_id, recipient_id):
    variable_index = sender_id[1:]
    factor_index = recipient_id[1:]
    outgoing_message = SumProductFactorNode(sender_id,
            messages).update_edge_message(recipient_id)
    return outgoing_message 

class SumProductNode():
...
class SumProductVariableNode(SumProductNode):
    def __init__(self, variable_id, incoming_messages, observation):
        SumProductNode.__init__(self, variable_id, incoming_messages)
    ...
    def __compute_message_product(self, edges):
        return reduce(lambda accum, new: np.multiply(accum, new),
                        [edge.message for edge in edges])
...
class SumProductFactorNode():
    def __init__(self, factor_id, incoming_messages):
        SumProductNode.__init__(self, factor_id, incoming_messages)
        '''
        if '4' in factor_id:
            self.incoming_messages = [AttrDict({
                'message': np.array([1-neighbor_message, neighbor_message]), 
...
#                incoming_messages.items()]
            ...
            }) for neighbor_id, neighbor_message in incoming_messages.items()]
    ...
    def compute_constraint_score(self, edges):
        ...
        marginal_values_vector = np.array([np.argmax(edge.message) for edge in
            sorted(edges, key=lambda edge: edge.variable_node_id)])
    ...
    def __node_state_from_edges(self, edges):
        neighbor_messages = sorted(edges, key=lambda edge: edge.variable_node_id)
        ...
        factor_function = lambda values: 1 if (factor_bounds[0] <=
                np.inner(factor_coeffs, values) <= factor_bounds[1]) else 0
        '''
        factor_function = edges[0].factor_function
    ...
    def __update_factor_edge(self, edge, index, samples, factor_function,
        ...
        edge.decimation_status = decimation_update
    ...
    def __compute_new_message_from_sample_vectors(self, index, sample_vectors,
        ...
        factor_function_samples = \
        [self.__apply_factor_function(index, sample_vector, factor_function) for
            sample_vector in sample_vectors]
