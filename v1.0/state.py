...
class NodeStateStore: #node state manager
    ...
    def update_node(self, incoming_message, node_id):
        self.state_store_spec.update_node(incoming_message, node_id)
...
class RedisNodeStateStore:
    ...
    def update_node_messages(self, updated_messages, node_id):
        redis = Redis()
        redis.hset(node_id, "messages", updated_messages)
        self.node_state_database[node_id] = incoming_message
        return True
